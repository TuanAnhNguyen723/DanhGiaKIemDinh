...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (password_verify($oldPass, $hashed_password)) {
        // Kiểm tra độ dài mật khẩu mới và xác nhận mật khẩu
        if (strlen($newPass) < 6 || strlen($confirm) < 6) {
            $newPass_err = "Mật khẩu phải có ít nhất 6 ký tự";
        } elseif ($newPass !== $confirm) {
            $confirm_err = "Mật khẩu mới và xác nhận mật khẩu không khớp";
        } else {
            // Mã hóa mật khẩu mới
            ...
            $update_stmt->close(); // Đóng lệnh cập nhật
        }
    } else {
        $oldPass_err = "Mật khẩu cũ không đúng, vui lòng nhập lại";
    }

    $stmt->close(); // Đóng lệnh truy vấn
